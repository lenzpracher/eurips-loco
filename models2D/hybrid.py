"""
2D Hybrid Implementation

This module provides the 2D Hybrid implementation for Navier-Stokes equation experiments.

The Hybrid model combines FNO and LOCO spectral approaches.
"""
...
# Import from the other model files for reuse
from .fno import NFNOSpectralConv2D
from .loco import LOCOSpectralConv2D
...
class FNOBlock2D(nn.Module):
    def __init__(self, hidden_channels, modes_x=16, modes_y=16, expansion_ratio=2, use_loco=False):
        ...
        if self.use_loco and hidden_channels % 2 != 0:
            raise ValueError("hidden_channels must be divisible by 2 when use_loco=True")

        if self.use_loco:
            half = hidden_channels // 2
            self.loco_conv = LOCOSpectralConv2D(half, half, modes_x, modes_y)
    ...
    def forward(self, x):
        identity = x
        if self.use_loco:
            x_loco, x_fno = torch.split(x, x.shape[1] // 2, dim=1)
            x_spec = torch.cat([
                self.loco_conv(x_loco),
                self.fourier(x_fno)
            ], dim=1)
        else:
...
class Hybrid(nn.Module):
    """
    Hybrid: FNO-LOCO hybrid architecture.

    This combines the FNO architecture with LOCO spectral convolutions.
    """
    def __init__(self, in_channels=1, out_channels=1, hidden_channels=32, modes_x=16, modes_y=16, n_layers=4):
        ...
        # FNO blocks with LOCO enabled for hybrid spectral learning
        self.blocks = nn.ModuleList([FNOBlock2D(hidden_channels, modes_x, modes_y, use_loco=True) for _ in range(n_layers)])
...
def create_ns2d_hybrid(
    ...
    """
    Create Hybrid model with NS2D configuration
    """
    return Hybrid(
