#!/usr/bin/env python3
"""
Burgers 1D Experiment

Neural operator training for the Burgers equation.

Usage:
    python burgers_1d.py --mode parallel-train
    python burgers_1d.py --mode parallel-plot
    python burgers_1d.py --mode rollout-loss
    python burgers_1d.py --mode space-time
"""
...
DATA_DIR = 'data/Burgers'  # Updated to use local data
...
MODES = 16
CHANNELS = 1
HIDDEN_CHANNELS = 24  # Reduced to balance with FNO parameter count
NUM_BLOCKS = 4
SPATIAL_POINTS = 512  # Burgers data has 512 spatial points
...
def train_model(model, train_loader, val_loader, optimizer, loss_fn, device, max_steps_per_epoch, model_type='sno'):
...
def save_seeds_summary(aggregated_losses, checkpoints_dir, num_runs):
    ...
    with open(summary_path, 'w') as f:
        f.write("Burgers Parallel Training Summary\n")
...
def run_plot_parallel():
    ...
    plot_losses(renamed_losses, title_suffix="Burgers",
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Train and evaluate models on Burgers' equation data.")
    ...
    parser.add_argument('--runs-per-gpu', type=int, default=6, help="Number of runs per GPU")
    ...
    parser.add_argument('--sample-id', type=int, default=100, help="Sample ID for space-time visualization (default: 100)")
    ...
    if args.mode == 'parallel-train':
        ...
        print("🔍 Running rollout loss analysis for Burgers equation...")
        ...
        device = torch.device(f'cuda:{args.gpu}' if torch.cuda.is_available() else 'cpu')
        print("🎨 Running space-time analysis for Burgers equation...")
        print(f"🎨 Device: {device}")
        print(f"🎨 Rollout steps: {args.rollout_steps}")
